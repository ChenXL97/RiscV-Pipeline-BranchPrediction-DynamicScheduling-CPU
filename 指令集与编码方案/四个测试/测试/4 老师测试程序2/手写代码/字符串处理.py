...
f1 = open('../init_file/datamem/datamem0.coe','w')
f2 = open('../init_file/datamem/datamem1.coe','w')
f3 = open('../init_file/datamem/datamem2.coe','w')
f4 = open('../init_file/datamem/datamem3.coe','w')
view = open('datamem_view.txt','w')
f = [f1,f2,f3,f4]
for x in f:
	x.write('memory_initialization_radix=2;\nmemory_initialization_vector=\n')

index,body,ip = process(find_string,4)
n = 0
for i in range(4):
	f[i].write('00000000'+'\n')
view.write(str(n) + ' : ' + '0000000000000000' + '\n')
n+=4
for x in body:
	for i in range(4):
...
index,body,ip = process(search_string,offset+4)
for x in body:
	for i in range(4):
		f[i].write(x[i*8:(i+1)*8]+'\n')
	view.write(str(n) + ' : ' + x + '\n')
